            ...
            if (lapsToEnd > 4)
            {
                ...
                if (penalty)
                {
                        ...
                        penalty->penalty == RM_PENALTY_STOPANDGO)
                    {
                        ...
                        pitNeeded = true;
#ifdef STRATEGY_DEBUG
                        fprintf(stderr, "%s serving %s penalty\n",car->_name,(stopPenalty==RM_PENALTY_STOPANDGO ? "Stop & Go" : "Drivethrough")); fflush(stderr);
#endif
                        return pitNeeded;
                    }
                }
            ...
            if (car->_dammage > m_maxDamage && isPitFree(car))
            {
                ...
                if (laps_to_go(car) > 5) {
                    ...
                    if (car->_dammage > MAX_DAMAGE) {
                        ...
                        if (!m_checkDamage) {
#ifdef STRATEGY_DEBUG
                            fprintf(stderr, "%s Dont Stop for Damage! Laps_toGo:%d  Car_damage: %d Max_damage: %d\n", car->_name, laps_to_go(car), car->_dammage, m_maxDamage); fflush(stderr);
#endif
                            m_checkDamage = true;
                            needRepair = false;
                        }
...
tCarPitCmd::TireChange SimpleStrategy::pitTyres(tCarElt *car, tSituation *s)
{
    ...
    /*            --- No point repeating all this logic, we've already looked at this elsewhere.
    int i;
    float mW = 0.0f, mG = 0.0f;
    ...
    int lapsToEnd = car->_remainingLaps - car->_lapsBehindLeader + 1;

    if ((mG > m_mGL || mW > m_mWL) && lapsToEnd > 2)
    {
        m_tROL = car->_laps;
        return tCarPitCmd::ALL;
    }

    float wPL = mW / (car->_laps - m_tROL);

    if (mW + wPL * (lapsToEnd-1) > m_mWL)
    {
        m_tROL = car->_laps;
        return tCarPitCmd::ALL;
    }

    if (mW > 0.5 && mG > 0.1 && (m_mWL - mG) / (wPL*4) > lapsToEnd)
    {
        m_tROL = car->_laps;
        return tCarPitCmd::ALL;
    }
    ...
    float aRT = (CT(0)+CT(1));
    if (mW > 0.5f && aRT < MIN(70.0f, 65.0f+(float)lapsToEnd))
    {
        m_tROL = car->_laps;
        return tCarPitCmd::ALL;
    }
