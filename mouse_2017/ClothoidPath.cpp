...
const ClothoidPath::Options&	ClothoidPath::GetOptions() const
{
	return m_options;
}
/*
...
void	ClothoidPath::AnalyseBumps( const CarModel& cm, bool dumpInfo )
{
	...
	{for( int count = 0; count < 2; count++ )
	{
		...
		for( int i = 0; i < NSEG; i++ )
		{
			...
			if( count == 1 && dumpInfo )
			{
				DEBUGF( "###  %4d  spd %3.0f k %7.4f dt %.3f pz %5.2f sz %5.2f vz %5.2f -> h %5.2f\n",
...
//		int Tries[MaxDivs];
		...
		for (i = 0; i < NSEG - IndexStep; i += IndexStep)
		{
			...
			MakeSmoothPath(m_pTrack, cm, Options());
			CalcMaxSpeeds( cm );
			PropagateBraking( cm );
			PropagateAcceleration( cm );
...
/*
				...
				if( idx >= 620 && idx <= 630 )
				{
					//OptimiseLine( cm, idx, step, l3, l2, l4 );
