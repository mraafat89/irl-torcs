...
//	if( (oCar->_state & (RM_CAR_STATE_NO_SIMU | RM_CAR_STATE_OUT)) )
	...
	NORM_PI_PI(oYaw);
	double	extSide = (m_info.sit.minDXa - m_info.sit.minDY) *
				(fabs(sin(myYaw)) + fabs(sin(oYaw)));
	double	extTan = rdPX > 0 && m_info.sit.agAX < -2 ? 0.25 : 0;
...
//	m_info.sit.minDX += 1.0;//pTeamInfo->IsTeamMate(myCar, oCar) ? 0.5 : 1.0;
	m_info.sit.minDXa += extTan + 2.0;//pTeamInfo->IsTeamMate(myCar, oCar) ? 0.5 : 1.0;
	m_info.sit.minDXb += extTan + 0.5;//pTeamInfo->IsTeamMate(myCar, oCar) ? 0.5 : 1.0;
	m_info.sit.decDX = m_info.sit.minDXb + 2;
	m_info.sit.accDX = m_info.sit.minDXb;

//	if( fabs(extSide) > 0.2 )
//		DEBUGF( "****** angDiff %.3f extSide %.2f ******\n", angDiff, extSide );
...
//		me.GetPtInfo( MyRobot::PATH_NORMAL, catPos * 0.5, pi );
		me.GetPtInfo( MyRobot::PATH_NORMAL, midPos * 0.5, pi );
