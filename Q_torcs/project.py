...
class Q_learn:
    ...
    def _initialize_Qs(self):
        if os.path.isfile(self.filepath_Q1):
            self.Q1 = np.load(self.filepath_Q1)
        else:
            self.Q1 = np.zeros((self.num_states,self.num_actions))

        if os.path.isfile(self.filepath_Q2):
            self.Q2 = np.load(self.filepath_Q2)
        else:
    ...
    def get_state(self,inputs):
      for i1 in range(len(inputs)):
        if inputs[i1] < self.ranges[i1][0]:
          ...
          print("here1", inputs[i1], self.ranges[i1])

        if inputs[i1] > self.ranges[i1][-1]:
          inputs[i1] = self.ranges[i1][-1]-1e-6
          print("here2", inputs[i1], self.ranges[i1])
