...
class Q_learn:
    ...
    def __init__(self,filepath_Q,alpha,epsilon):
        ...
        self.num_states = 14*6*6*6*9*9*10
        self._initialize_Q()
    ...
    def _initialize_Q(self):
        ...
        if os.path.isfile("results2/singleQ_rewards.npy"):
            self.rewards = np.load("results2/singleQ_rewards.npy").tolist()
            self.distances = np.load("results2/singleQ_distances.npy").tolist()
            self.angles = np.load("results2/singleQ_angles.npy").tolist()
            self.times = np.load("results2/singleQ_times.npy").tolist()
    ...
    def save_Q(self):
        ...
        np.save("results2/singleQ_rewards",self.rewards)
        np.save("results2/singleQ_distances",self.distances)
        np.save("results2/singleQ_angles",self.angles)
        np.save("results2/singleQ_times",self.times)
    ...
    def get_state(self,inputs):
        ...
        ray1 = self.disc_track(inputs[2])   #6
        ...
        return angle*(6*6*6*9*9*10) + ray0*(6*6*9*9*10) + ray0*(6*9*9*10) + ray2*(9*9*10) + pos*(9*10) + sx*(10) + sy
