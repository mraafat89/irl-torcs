...
class TorcsEnv:
    ...
    def __init__(self, vision=False, throttle=False, gear_change=False, display=False, track_id=1):
        ...
        if self.vision is True:
            ...
            if self.display is True:
                ...
                os.system(
                    'torcs -r ' + cur_dir + '/race_config/track-' + str(self.track_id) + '.xml -nofuel -nolaptime &')

        time.sleep(0.5)
        # Modify here if you use multiple tracks in the environment
        self.client = snakeoil3.Client(p=3001, vision=self.vision, display=self.display,
                                       track_id=self.track_id)  # Open new UDP in vtorcs
    ...
    def step(self, u):
        ...
        stepTime = t - self.past_t
        self.past_t = t

        d = obs['distFromStart'] - self.past_d
        past_d = obs['distFromStart']

        accX = self.KPH_to_MPS * (sp - self.prevSpeedX) / stepTime
        jerkX = (accX - self.prevAccX) / stepTime
        snapX = (jerkX - self.prevJerkX) / stepTime

        accY = self.KPH_to_MPS * (np.array(obs['speedY']) - self.prevSpeedY) / stepTime
        jerkY = (accY - self.prevAccY) / stepTime
        snapY = (jerkY - self.prevJerkY) / stepTime
        ...
        reward = (sp-20) * (0.1 - np.abs(obs['angle'])) + (sp-20) * (0.3 - np.abs(obs['trackPos']))
        ...
        self.prevSpeedX = sp
        self.prevAccX = accX
        self.prevJerkX = jerkX

        self.prevSpeedY = np.array(obs['speedY'])
        self.prevAccY = accY
        self.prevJerkY = jerkY
